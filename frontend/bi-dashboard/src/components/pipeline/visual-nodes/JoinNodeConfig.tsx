...
import { Select, Button, Divider, Tag, Typography, Alert, Card, Spin } from 'antd';
...
export const JOIN_TYPES = [
  ...
  {
    value: 'symmetric_diff',
    label: 'Symmetric diff',
    description: '仅左无匹配 ∪ 仅右无匹配（两侧列结构需一致才能 UNION）',
    color: '#64748B',
  },
];
...
export const JoinNodeConfig: React.FC<JoinNodeConfigProps> = ({
  ...
  useEffect(() => {
    ...
    leftPreview.loadPreview({ node: leftNode, allNodes, pipelineDataSourceId: leftDs, limit: 80 }, true);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- 仅上游或数据源变化时重拉
  }, [leftNode?.id, leftDs, allNodes]);

  useEffect(() => {
    if (!rightNode || !rightDs) return;
    rightPreview.loadPreview({ node: rightNode, allNodes, pipelineDataSourceId: rightDs, limit: 80 }, true);
  ...
  const writeJoinState = useCallback(
    (nextRegions: JoinVennRegions, nextKeys: JoinKey[]) => {
      ...
      const symBuilt =
        jt === 'symmetric_diff' ? buildSymmetricUnionPlan(leftColsRef.current, rightColsRef.current) : [];
  ...
  if (upstreamNodes.length < 2) {
    return (
      <Alert
        ...
        description={
          <div>
            关联节点需要至少两个上游数据源。 当前已连接：{upstreamNodes.length} 个。 请先添加更多节点并连接到本节点。
          </div>
        }
  ...
  return (
    <div>
      ...
      {joinKeys.map((key) => (
        <Card key={key.id} size="small" style={{ marginBottom: 8 }} bodyStyle={{ padding: '8px 10px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <Tag style={{ background: '#BAE6FD', color: '#0EA5E9', border: 'none', fontSize: 11 }}>左表</Tag>
            <Select
              size="small"
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 6 }}>
            <Tag style={{ background: '#DDD6FE', color: '#8B5CF6', border: 'none', fontSize: 11 }}>右表</Tag>
            <Select
              size="small"
            ...
            {!readOnly && joinKeys.length > 1 && (
              <Button size="small" danger icon={<DeleteOutlined />} onClick={() => removeJoinKey(key.id)} />
            )}
          </div>
        </Card>
      ))}
      ...
      >
        {validKeys.length === 0 ? (
          ...
          <span>
            ...
            {validKeys
              .map(
                (k) => `a.\`${k.leftCol}\` COLLATE utf8mb4_unicode_ci = b.\`${k.rightCol}\` COLLATE utf8mb4_unicode_ci`,
              )
              .join(' AND ')}
            <br />
          ...
          <span>
            SELECT a.* … WHERE b.`{validKeys[0].rightCol}` IS NULL
            <br />
            &nbsp;&nbsp;UNION ALL
            <br />
          ...
          <span>
            ...
            &nbsp;&nbsp;{JOIN_TYPES.find((j) => j.value === joinType)?.label ?? joinType.toUpperCase()} JOIN right_table
            b<br />
            &nbsp;&nbsp;ON{' '}
            {validKeys
              .map(
