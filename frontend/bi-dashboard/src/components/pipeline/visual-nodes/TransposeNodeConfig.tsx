...
export const TransposeNodeConfig: React.FC<TransposeNodeConfigProps> = ({
  ...
  // ── 过滤掉已选为索引/透视的列后剩余可选列 ──────────────────────
  const selectableForValue = upstreamColumns.filter(
    c => c.name !== pivotColumn && !indexColumns.includes(c.name)
  );
    ...
    <div>
      ...
      {loading && (
        <div style={{ textAlign: 'center', padding: 8 }}>
          <Spin size="small" /> <Text type="secondary" style={{ fontSize: 12, marginLeft: 8 }}>加载上游数据中…</Text>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          <Text strong style={{ fontSize: 13 }}>
            <SwapOutlined style={{ marginRight: 6 }} />索引列
          </Text>
          <Text type="secondary" style={{ fontSize: 11, marginLeft: 4 }}>（GROUP BY）</Text>
          <Select
            mode="multiple"
            placeholder="选择索引列"
            value={indexColumns}
              ...
              label: (
                <Space size={4}>
                  <Tag style={{
                    ...
                    border: 'none', fontSize: 9, padding: '0 3px',
                  }}>{getDataTypeInfo(c.type).label}</Tag>
                  <span>{c.name}</span>
                </Space>
              ),
            }))}
        ...
        <div style={{ flex: 1 }}>
          <Text strong style={{ fontSize: 13 }}>
            <SwapOutlined style={{ marginRight: 6 }} />透视列
          </Text>
          <Text type="secondary" style={{ fontSize: 11, marginLeft: 4 }}>（取唯一值）</Text>
          <Select
            placeholder="选择透视列"
      ...
      <div style={{ marginBottom: 12 }}>
        <Text strong style={{ fontSize: 13 }}>
          <SwapOutlined style={{ marginRight: 6 }} />透视值
        </Text>
      ...
      <div style={{ marginBottom: 12 }}>
        <Text strong style={{ fontSize: 13 }}>
          <SwapOutlined style={{ marginRight: 6 }} />值列
        </Text>
        <Text type="secondary" style={{ fontSize: 11, marginLeft: 4 }}>（聚合到各透视列下）</Text>
...
function buildTransposeSql(
  ...
  for (const vc of valueCols) {
    for (const pv of pivotVals) {
      ...
      caseParts.push(
        `${vc.aggMethod}(CASE WHEN \`${pivotCol}\` = '${safeVal}' THEN ${col} END) AS \`${alias}\``
      );
    }
  }
