...
import {
  Drawer, Form, Select, Input, Button, Space, Collapse, Tag, Divider
} from 'antd';
      ...
      footer={
        <div style={{ textAlign: 'right' }}>
          <Space>
            ...
            <Button
              type="primary"
              disabled={selectedUpstream.length < 2}
      ...
      <Form layout="vertical">
        ...
        {selectedUpstream.length >= 2 && (
          <div style={{ marginTop: 8 }}>
            ...
            >
              <Panel
                header={<span><InfoCircleOutlined style={{ marginRight: 6 }} />SQL 模板预览</span>}
                key="sql-template"
              >
                <div style={{ marginBottom: 8 }}>
                  ...
                  <ul style={{ fontSize: 12, color: '#666', paddingLeft: 20 }}>
                    <li><code>{'{upstream_table_0}'}</code> — 第一个上游节点的结果表</li>
                    <li><code>{'{upstream_table_1}'}</code> — 第二个上游节点的结果表</li>
                    <li><code>{'{upstream_table_<node_id>'}</code> — 按节点 ID 引用</li>
                    <li><code>{'{prev_table}'}</code> — 等同于第一个上游引用</li>
                  </ul>
                </div>
                {renderSqlTemplate()}
              </Panel>
