...
export function useNodePreview() {
  ...
  const loadPreview = useCallback(async (params: LoadPreviewParams, immediate = false): Promise<PreviewData | null> => {
      ...
      debounceRef.current = setTimeout(async () => {
        try {
          ...
          const graph_nodes = allNodes.map((n: GraphNode) => {
            ...
            const baseCfg = pn.config && typeof pn.config === 'object' ? { ...pn.config } : {};
