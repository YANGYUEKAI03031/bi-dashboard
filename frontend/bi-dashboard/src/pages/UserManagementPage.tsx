          ...
          <div>
            <div style={{ marginBottom: 24 }}>
              ...
              <Select
                ...
                onChange={(ids: number[]) => handleBatchPermissionChange(ids, reportPermissions.filter(p => p.can_edit).map(p => p.report_page_id))}
            ...
            <div>
              ...
              <Select
                ...
                onChange={(ids: number[]) => handleBatchPermissionChange(reportPermissions.filter(p => p.can_view).map(p => p.report_page_id), ids)}
                options={reportPages.map(p => ({ label: p.name, value: p.id }))}
              />
            </div>
          </div>
