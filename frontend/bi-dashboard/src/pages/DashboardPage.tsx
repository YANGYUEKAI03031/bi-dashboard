...
const requestMonitor = {
  ...
  logRequest: (chartId: number, type: 'cache' | 'network') => {
    ...
    // Request logged: [${elapsed}ms] chart ${chartId} (${type})
  },
};
...
export const DashboardPage: React.FC = () => {
  ...
  const renderDraggableChartItem = (chart: Chart) => (
    ...
    >
      <Card size="small" hoverable onClick={() => handleAddChartToDashboard(chart.id)}>
  ...
  const DashboardCardComponent: React.FC<{ card: DashboardCard }> = ({ card }) => {
      ...
      return (
        ...
        >
          <div style={{ textAlign: 'center', color: '#ff4d4f' }}>
            <p>图表数据缺失</p>
            <Button type="link" danger onClick={() => handleRemoveChartFromDashboard(card.id)}>
              移除卡片
            </Button>
          </div>
        </div>
    ...
    return (
      <div key={card.id} className="dashboard-card-wrapper" style={{ height: '100%', position: 'relative' }}>
        ...
        <Card
          title={
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '12px', color: '#888', marginTop: 4 }}>{card.chart.chart_type}</div>
            </div>
          }
          extra={
            <Button
        ...
        >
          {dataLoading ? (
            ...
            >
              <Spin tip="加载数据中..." />
              <div style={{ marginTop: '8px', fontSize: '12px', color: '#888' }}>正在执行查询...</div>
            </div>
          ) : error ? (
            <div
              style={{
            ...
            >
              <div style={{ textAlign: 'center' }}>
                ...
                <div>暂无数据</div>
                <div style={{ fontSize: '12px', marginTop: '4px' }}>查询返回空结果</div>
              </div>
            </div>
          ) : (
            <div className="chart-container">
              {(() => {
                ...
                const xField =
                  viz.x_field ?? (Array.isArray(viz.graph_dimensions) ? viz.graph_dimensions[0] : undefined);
                ...
                const yAggMethod = viz.y_agg_method ?? viz['graph.y_agg_method'] ?? undefined;
        ...
        <Row gutter={24}>
          ...
          <Col span={6}>
            <Card
              ...
              extra={
                <Button type="link" size="small" onClick={loadCharts} loading={loading}>
                  刷新
                </Button>
              }
            >
              {charts.length === 0 ? (
                ...
                >
                  <div style={{ marginBottom: '8px' }}>📊</div>
                  <div>暂无可用图表</div>
                  <div style={{ fontSize: '12px', marginTop: '4px' }}>请先创建图表</div>
                  <Button
                    type="primary"
                    size="small"
                ...
                <div className="chart-search-panel">
                  <Select
                    ...
                    dropdownRender={() => (
                      <div className="chart-search-dropdown">
                        <div className="chart-search-dropdown-hint">提示：拖拽图表到右侧画布即可添加</div>
                  ...
                  <div className="chart-search-below">
                    ...
                    <div className="chart-list">{charts.map((chart) => renderDraggableChartItem(chart))}</div>
                  </div>
                </div>
              )}
            </Card>
                ...
                <div className="dashboard-grid">
                  <AutoWidthGridLayout
                    ...
                    onDrop={(layout: GridLayoutItem[], item: GridLayoutItem, e: DragEvent) => {
                      try {
                        const raw = e.dataTransfer?.getData('chartId') || e.dataTransfer?.getData('text/plain') || '';
