...
from app.models.dashboard import Dashboard, DashboardCard, DashboardFilter, DashboardFilterBinding
...
class DashboardService:
    ...
    async def update_dashboard(
        self, dashboard_id: int, dashboard_data: DashboardUpdate, user_id: int
    ) -> Dashboard | None:
        """更新仪表板基本信息/布局/设置"""
        try:
            ...
            if update_fields:
                upd = update(Dashboard).where(Dashboard.id == dashboard_id).values(update_fields)
    ...
    async def get_dashboard(self, dashboard_id: int, user_id: int) -> Dashboard | None:
        """获取仪表板详情（包含卡片和图表信息）。创建者、管理员、或通过报表授权可查看/编辑的用户均可访问。"""
        try:
            # 使用selectinload预加载关联数据，先不限制 creator_id
            stmt = (
                select(Dashboard)
                .options(selectinload(Dashboard.dashboard_cards).selectinload(DashboardCard.chart))
            ...
            if dashboard:
                ...
                for card in dashboard.cards:
                    if not hasattr(card, "chart") or card.chart is None:
                        # 如果关联数据没有正确加载，手动查询
                        chart_stmt = select(VisualizationCard).where(VisualizationCard.id == card.chart_id)
    ...
    async def add_chart_to_dashboard(
        self, dashboard_id: int, card_data: DashboardCardCreate, user_id: int
    ) -> DashboardCard:
        """向仪表板添加图表"""
        try:
            # 验证仪表板属于用户
            ...
            if (
                ...
                and not await perm_service.can_view_chart_via_report_page(user_id, chart.id)
            ):
                raise Exception("图表不存在或无权限访问")
    ...
    async def update_dashboard_card(
        self, card_id: int, update_data: DashboardCardUpdate, user_id: int
    ) -> DashboardCard | None:
        """更新仪表板卡片位置和设置。创建者、管理员、或通过报表可编辑的用户均可操作。"""
        try:
            card_stmt = (
                select(DashboardCard)
                .options(selectinload(DashboardCard.chart), selectinload(DashboardCard.dashboard))
            ...
            if (
                ...
                and not await perm_service.is_admin(user_id)
                and not await perm_service.can_edit_dashboard(user_id, dashboard.creator_id, dashboard_id=dashboard.id)
            ):
                return None
            ...
            if update_fields:
                stmt = update(DashboardCard).where(DashboardCard.id == card_id).values(update_fields)
    ...
    async def remove_chart_from_dashboard(self, card_id: int, user_id: int) -> bool:
        ...
        try:
            card_stmt = (
                select(DashboardCard).options(selectinload(DashboardCard.dashboard)).where(DashboardCard.id == card_id)
            )
            card_result = await self.db.execute(card_stmt)
            card = card_result.scalar_one_or_none()
    ...
    async def delete_dashboard(self, dashboard_id: int, user_id: int) -> bool:
        ...
        try:
            ...
            # 删除报表页与仪表盘的关联记录
            delete_rpd_stmt = delete(ReportPageDashboard).where(ReportPageDashboard.dashboard_id == dashboard_id)
            await self.db.execute(delete_rpd_stmt)
            logger.info(f"已删除仪表盘 {dashboard_id} 的所有报表页关联记录")

            # 软删除：标记为已归档
            stmt = update(Dashboard).where(Dashboard.id == dashboard_id).values(archived=True, updated_at=utc_now())
    ...
    async def create_filter(
        self, dashboard_id: int, filter_data: DashboardFilterCreate, user_id: int
    ) -> DashboardFilter:
        """创建筛选器"""
        try:
            # 验证仪表板属于用户
            dashboard_stmt = select(Dashboard).where(Dashboard.id == dashboard_id, Dashboard.creator_id == user_id)
            dashboard_result = await self.db.execute(dashboard_stmt)
            dashboard = dashboard_result.scalar_one_or_none()
    ...
    async def _auto_bind_filter_to_charts(self, filter_obj: DashboardFilter, dashboard_id: int):
        ...
        try:
            cards_stmt = select(DashboardCard).where(DashboardCard.dashboard_id == dashboard_id)
            ...
            for card in cards:
                ...
                existing_binding_stmt = select(DashboardFilterBinding).where(
                    DashboardFilterBinding.filter_id == filter_obj.id, DashboardFilterBinding.card_id == card.id
                )
    ...
    async def get_filters(self, dashboard_id: int, user_id: int) -> list[DashboardFilter]:
        ...
        try:
            ...
            if (
                dashboard.creator_id != user_id
                and not await perm_service.is_admin(user_id)
                and not await perm_service.can_view_dashboard_via_report_page(user_id, dashboard_id)
            ):
            ...
            stmt = (
                ...
                .options(selectinload(DashboardFilter.bindings))
    ...
    async def get_filter(self, filter_id: int, user_id: int) -> DashboardFilter | None:
        """获取单个筛选器详情"""
        try:
            stmt = (
                select(DashboardFilter)
                .options(selectinload(DashboardFilter.bindings))
                .join(Dashboard)
                .where(DashboardFilter.id == filter_id, Dashboard.creator_id == user_id)
            )
    ...
    async def update_filter(
        self, filter_id: int, filter_data: DashboardFilterUpdate, user_id: int
        ...
        try:
            ...
            if update_fields:
                stmt = update(DashboardFilter).where(DashboardFilter.id == filter_id).values(update_fields)
    ...
    async def bind_filter_to_card(
        self, filter_id: int, binding_data: DashboardFilterBindingCreate, user_id: int
    ) -> DashboardFilterBinding | None:
        """手动绑定筛选器到图表卡片"""
        try:
            # 验证筛选器属于用户的仪表板
            filter_obj = await self.get_filter(filter_id, user_id)
            ...
            card_stmt = (
                select(DashboardCard)
                .join(Dashboard)
                .where(DashboardCard.id == binding_data.card_id, Dashboard.creator_id == user_id)
            )
            card_result = await self.db.execute(card_stmt)
            card = card_result.scalar_one_or_none()
            ...
            existing_stmt = select(DashboardFilterBinding).where(
                DashboardFilterBinding.filter_id == filter_id, DashboardFilterBinding.card_id == binding_data.card_id
            )
            ...
            if existing_binding:
                ...
                # 创建新绑定
                binding = DashboardFilterBinding(
                    filter_id=filter_id, card_id=binding_data.card_id, param_name=binding_data.param_name
                )
                self.db.add(binding)
                await self.db.commit()
    ...
    async def unbind_filter_from_card(self, filter_id: int, card_id: int, user_id: int) -> bool:
        ...
        try:
            ...
            delete_stmt = delete(DashboardFilterBinding).where(
                DashboardFilterBinding.filter_id == filter_id, DashboardFilterBinding.card_id == card_id
            )
            await self.db.execute(delete_stmt)
            await self.db.commit()
    ...
    async def bind_filter_to_new_card(self, card_id: int, dashboard_id: int):
        ...
        try:
            filters_stmt = select(DashboardFilter).where(DashboardFilter.dashboard_id == dashboard_id)
            ...
            for filter_obj in filters:
                ...
                existing_stmt = select(DashboardFilterBinding).where(
                    DashboardFilterBinding.filter_id == filter_obj.id, DashboardFilterBinding.card_id == card_id
                )
                existing_result = await self.db.execute(existing_stmt)
                existing_binding = existing_result.scalar_one_or_none()
