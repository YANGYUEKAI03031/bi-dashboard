...
@router.get("/users")
async def list_users_with_roles(db: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    """获取所有用户及其角色（仅管理员可访问）"""
    try:
        ...
        if not await service.is_admin(user_id):
            raise PermissionDeniedException("查看用户列表")
        ...
        raise DatabaseException("获取用户列表失败", original_error=e)


@router.post("/users", response_model=CreateUserResponse)
async def create_user(
    payload: CreateUserRequest, db: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)
):
    """创建新账号（仅管理员可访问）"""
    try:
        service = PermissionService(db)
        if not await service.is_admin(user_id):
            raise PermissionDeniedException("创建用户")
        ...
        raise DatabaseException("创建用户失败", original_error=e)


@router.put("/users/{target_user_id}/role")
async def set_user_role(
    target_user_id: int, role: str, db: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)
):
    """设置用户角色（仅管理员可访问）"""
    try:
        service = PermissionService(db)
        ...
        # 检查是否为管理员
        if not await service.is_admin(user_id):
            raise PermissionDeniedException("设置用户角色")
        ...
        raise DatabaseException("设置用户角色失败", original_error=e)


@router.get("/my-role")
async def get_my_role(db: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    """获取当前用户角色"""
    service = PermissionService(db)
...
async def get_my_permissions(db: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    """获取当前用户的权限信息"""
    service = PermissionService(db)
    return {
        "user_id": user_id,
...
async def grant_report_page_view(
    ...
    try:
        ...
        raise DatabaseException("授权报表权限失败", original_error=e)
...
async def revoke_report_page_view(
    ...
    try:
        ...
        # 权限检查
        if not await service.is_admin(user_id) and report.creator_id != user_id:
            raise PermissionDeniedException("撤销此报表的授权")
        ...
        if success:
            ...
            raise ResourceNotFoundException("权限记录")

    except Exception as e:
        logger.error(f"撤销报表权限错误: {str(e)}")
        raise DatabaseException("撤销报表权限失败", original_error=e)


@router.get("/report-pages/{report_page_id}/permissions")
async def get_report_page_permissions(
    report_page_id: int, db: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)
):
    ...
    try:
        ...
        # 权限检查
        if not await service.is_admin(user_id) and report.creator_id != user_id:
            raise PermissionDeniedException("查看此报表的权限列表")
        ...
        raise DatabaseException("获取报表权限列表失败", original_error=e)
...
async def get_modification_logs(
    resource_type: str | None = Query(None, description="资源类型: chart, dashboard, report_page"),
    ...
    try:
        ...
        if not await service.is_admin(current_user_id):
            raise PermissionDeniedException("查看修改记录")

        logs = await service.get_modification_logs(
            resource_type=resource_type, resource_id=resource_id, user_id=user_id, limit=limit, offset=offset
        )
        return logs

    except Exception as e:
        logger.error(f"获取修改记录错误: {str(e)}")
        raise DatabaseException("获取修改记录失败", original_error=e)
...
async def get_resource_logs(
    ...
    try:
        ...
        if not await service.is_admin(user_id) and resource_creator_id != user_id:
            raise PermissionDeniedException("查看此资源的修改记录")
        ...
        raise DatabaseException("获取资源修改记录失败", original_error=e)
