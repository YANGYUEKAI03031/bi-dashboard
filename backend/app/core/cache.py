...
class EnhancedCacheService:
    ...
    async def get_cached_dataset(self, cache_key: str) -> dict[str, Any] | None:
        """获取缓存的数据集"""
    ...
    async def cache_dataset(self, cache_key: str, dataset_data: dict[str, Any], ttl: int = None) -> bool:
        """缓存数据集"""
        if not self.connected or not self.redis_client:
            return False
        ...
        try:
            ...
            # 存储到Redis
            await self.redis_client.setex(cache_key, ttl or self.default_ttl, serialized_data)
...
def cached_dataset(ttl: int = 3600):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            # 生成缓存键
            query_hash = hashlib.md5(f"{str(args)}{str(kwargs)}".encode()).hexdigest()
